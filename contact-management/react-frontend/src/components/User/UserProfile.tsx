import { userCurrent, userUpdateName, userUpdatePassword } from "../../lib/api/userApi"
...
export default function UserProfile() {
    ...
    async function handleUpdateName(e: SubmitEvent) {
        e.preventDefault();
        try {
            const response = await userUpdateName({ name });
            if (response.status == 200) {
                return successAlert('Update Name Successfully')
            }
        } catch (error) {
            if (error instanceof AxiosError && error.status === 400) {
                return errorAlert(error.response?.data.errors[0].name)
            }
            if (error instanceof Error) {
                return errorAlert(error.message);
            }
    ...
    async function handleUpdatePassword(e: SubmitEvent) {
        ...
        try {
            const response = await userUpdatePassword({ password });
            if (response.status == 200) {
                setPassword('');
                setPasswordConfirmation('');
                return successAlert('Update Password Successfully')
            }
        } catch (error) {
            if (error instanceof AxiosError && error.status === 400) {
                return errorAlert(error.response?.data.errors[0].password)
            }
            if (error instanceof Error) {
                return errorAlert(error.message);
            }
